...
export const loginUser = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    try {
      const response = (await axios.post(`${API_URL}/login`, userData)).data;
      // Return both the data and the message from the backend
      return {
        ...
        store: response.data.role === "MERCHANT" ? response.data.storeDetails : null,
        message: response.message, // Add the message from the backend
      };
    } catch (error) {
      return rejectWithValue(
